...
const publicHeader = () => {
  ...
  const parent = {auth:'welcome', reset:'auth', connect:caregiver ? 'invite' : 'welcome', consent:'connect', chat:'welcome', connection:'chat', finish:'welcome'}[route];
...
function shell(content, active = 'family') {
  return `<div class="shell"><header class="topbar">${brand()}<button class="text-button" data-action="logout">Exit demo</button></header>
    <main id="main" tabindex="-1" class="main-content">${content}<p class="footer-note">Fictional demo · Not a health assessment.</p></main>
    <nav class="tab-bar" aria-label="Caregiver navigation">
      ${[['family','people','Family'],['dashboard','home','Overview'],['observations','note','Notes']].map(([route,symbol,label]) => `<a class="nav-link ${active === route ? 'active' : ''}" ${active === route ? 'aria-current="page"' : ''} href="#${route}">${icon(symbol)}<span>${label}</span></a>`).join('')}
    </nav></div>`;
}
...
function invite() {
  return shell(`<div class="page-heading"><div><p class="eyebrow">Grow your care circle</p><h1>Bring someone closer.</h1><p class="muted">One invitation, for one loved one.</p></div></div><div class="form-grid"><section class="card">${invitation ? `<h2>Your invitation for ${escape(pendingPerson.name)}</h2><p>Ask them to choose “I’d like to talk” and enter this code on their phone.</p><div class="code">${invitation.code}</div><p class="sub">Expires ${new Date(invitation.expires).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})} · One use · Waiting for consent</p><div class="row"><button data-action="copy">Copy code</button><button class="secondary" data-action="regenerate">New code</button></div><p id="copy-status" role="status"></p><hr style="border:0;border-top:1px solid var(--line);margin:25px 0"><p class="notice">Demo linking works on this iPhone only. Use the button below to preview their side.</p><a class="button wide" href="#connect">Try their invitation ${icon('arrow')}</a>` : `<h2>Who would you like to link?</h2><form id="invite-form"><div class="field"><label for="adult-name">Their first name</label><input id="adult-name" maxlength="40" placeholder="For example, Sam" required></div><div class="field"><label for="relationship">Relationship</label><select id="relationship"><option>Parent</option><option>Partner</option><option>Relative</option><option>Friend</option></select></div><button type="submit">Create invitation ${icon('arrow')}</button></form>`}</section><section class="card"><h2>A connection they choose.</h2><ol class="steps"><li>You share their invitation code.</li><li>They check your name and what will be shared.</li><li>They agree to connect, with no password to remember.</li></ol><p class="sub">The planned product remembers their own device and lets them disconnect. This demo resets when the app restarts.</p></section></div>`);
}
function connect() {
